...
function buildHtml(p: BookingConfirmationRequest): string {
  const rows = [
    ["Booking #", p.booking_id],
    ["Move Date", p.move_date],
    p.arrival_window ? ["Arrival Window", p.arrival_window] : null,
    ["From", p.from_address],
    ["To", p.to_address],
    p.crew_size ? ["Crew Size", p.crew_size] : null,
    p.estimated_duration ? ["Estimated Duration", p.estimated_duration] : null,
    p.deposit_amount ? ["Deposit", p.deposit_amount] : null,
    p.balance_due ? ["Balance Due", p.balance_due] : null,
  ]
    .filter(Boolean)
    .map(
      (r) =>
...
<p style="color:#94a3b8;margin:4px 0 0;font-size:14px">Booking Confirmed</p>
</div>
<div style="padding:24px">
<p style="color:#1e293b;font-size:16px">Hi ${p.customer_name},</p>
<p style="color:#475569;font-size:14px;line-height:1.6">Your move has been confirmed! Here are the details:</p>
...
<p style="margin:0;color:#065f46;font-size:13px"><strong>What to expect:</strong> Our team will arrive during the scheduled window. Please ensure pathways are clear and any fragile items are marked.</p>
</div>
</div>
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!body.to || !body.customer_name || !body.booking_id || !body.move_date || !body.from_address || !body.to_address) {
      return new Response(
        JSON.stringify({ error: "Missing required fields: to, customer_name, booking_id, move_date, from_address, to_address" }),
    ...
    await client.send({
      ...
      to: body.to,
      subject: `Booking Confirmed - ${body.booking_id}`,
      content: `Hi ${body.customer_name}, your booking ${body.booking_id} for ${body.move_date} is confirmed.`,
      html: buildHtml(body),
    });
    await client.close();
    ...
    await supabase.from("audit_log").insert({
      ...
      metadata: { to: body.to, booking_id: body.booking_id },
      success: true,
    });
